...
import { DeclarationBlock, DescribesAtRule, NestedAtRule, ParseOptions, RegularAtRule, RuleSet, StyleSheet } from "../types";
...
function parseSelectors(subject:string, transformFunc?: (v:string) => string):string[] {
    ...
    return selectors.map(selector => {
        return StrKits.replaceRange(selector, detectAllClassAndId(selector).map(({start, end}) => {
            return {
                start,
                end,
 ...
 */
function parseDeclarationBlock(subject:string, transformFunc?:(v:string) => string):DeclarationBlock{
...
function parseAtRule(subject:string, options:ParseOptions):RegularAtRule | DescribesAtRule | NestedAtRule{
    ...
    switch(Checker.typeOfAtRule(identifier)){
        ...
        case 'nested': {
            ...
            return {
                ...
                identifier,
                rule,
                styleSheet: parseStyleSheetList(standardStyleSheet, options),
            }
        }
        default: {
            throw new SyntaxError(subject);
